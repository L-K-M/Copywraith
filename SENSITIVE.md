...
## Content that is never captured

Before any of that, the desktop clipboard monitor skips content whose source
marked it private. Password managers publish these markers next to a copied
secret so that clipboard managers leave it alone:

| Platform | Markers |
|---|---|
| macOS | `org.nspasteboard.ConcealedType`, `org.nspasteboard.TransientType`, `org.nspasteboard.AutoGeneratedType`, and the older `com.agilebits.onepassword`, `de.petermaurer.TransientPasteboardType`, `net.antelle.keeweb`, `Pasteboard generator type` |
| Windows | `ExcludeClipboardContentFromMonitorProcessing`, `Clipboard Viewer Ignore`, or `CanIncludeInClipboardHistory` set to 0 |
| Linux (X11 and Wayland) | `x-kde-passwordManagerHint` |

Marked content is not read, stored, or synced, so it never reaches the
heuristics below. The check lives in `NativeClipboard::read`
(`src-tauri/src/native_clipboard.rs`). Android capture does not check
`ClipDescription.EXTRA_IS_SENSITIVE` yet.
