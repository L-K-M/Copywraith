...
**Signing/notarization caveats:** the **macOS bundles are unsigned and un-notarized by design** — `release.yml` deliberately does not pass any `APPLE_*` secrets to `tauri-action`. macOS will warn on first launch; users open the app via right-click → Open, or clear the quarantine attribute.
