...
pub(crate) fn track(window: &WebviewWindow) -> tauri::Result<()> {
    ...
    // Match GTK decorations: move/resize grabs can take keyboard focus while
    ...
    native.connect_window_state_event(move |_, event| {
        if event.changed_mask().contains(WindowState::FOCUSED) {
            ...
            let _ = window.emit_to(window.label(), ACTIVITY_CHANGED, active);
        }
        Propagation::Proceed
    });
    Ok(())
...
pub(crate) fn track(window: &WebviewWindow) -> tauri::Result<()> {
    ...
    window.on_window_event(move |event| {
        if let tauri::WindowEvent::Focused(active) = event {
            let _ = emitter.emit_to(emitter.label(), ACTIVITY_CHANGED, *active);
        }
    });
    Ok(())
}
