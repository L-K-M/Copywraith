...
/// Formats whose mere presence marks clipboard content as private.
///
/// Password managers publish these next to a copied secret so that clipboard
/// managers leave it alone. macOS apps follow the nspasteboard.org convention
/// (plus a few older vendor types that predate it).
#[cfg(target_os = "macos")]
const PRIVATE_CONTENT_MARKERS: &[&str] = &[
...
/// Windows password managers (KeePass, 1Password) register these formats.
...
/// KeePassXC and KWallet set this MIME type (to "secret") on X11 and Wayland.
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
const PRIVATE_CONTENT_MARKERS: &[&str] = &["x-kde-passwordManagerHint"];

/// Windows clipboard history skips content whose DWORD value for this format
/// is zero; a non-zero value explicitly allows it.
#[cfg(target_os = "windows")]
const CLIPBOARD_HISTORY_OPT_OUT: &str = "CanIncludeInClipboardHistory";
...
pub(crate) enum ClipboardPayload {
    ...
    /// The source asked clipboard managers not to record this content.
    Private,
    Empty,
}
...
impl NativeClipboard {
    ...
    pub(crate) fn read(&self) -> Result<ClipboardPayload> {
        ...
        // Check before reading any payload so a secret never leaves the
        // pasteboard, not even into memory that is dropped afterwards.
        if is_marked_private(&context) {
            return Ok(ClipboardPayload::Private);
        }
...
fn is_marked_private(context: &ClipboardContext) -> bool {
    let has_marker = PRIVATE_CONTENT_MARKERS
        .iter()
        .any(|marker| context.has(ContentFormat::Other((*marker).to_string())));

    #[cfg(target_os = "windows")]
    let has_marker = has_marker
        || (context.has(ContentFormat::Other(CLIPBOARD_HISTORY_OPT_OUT.to_string()))
            && context
                .get_buffer(CLIPBOARD_HISTORY_OPT_OUT)
                .is_ok_and(|value| history_opt_out_denies(&value)));

    has_marker
}

/// Interpret the `CanIncludeInClipboardHistory` DWORD: zero denies recording.
/// A malformed empty value is treated as a denial, erring toward privacy.
#[cfg(any(target_os = "windows", test))]
fn history_opt_out_denies(value: &[u8]) -> bool {
...
mod read_tests {
    ...
    #[test]
    fn clipboard_history_opt_out_only_denies_a_zero_value() {
